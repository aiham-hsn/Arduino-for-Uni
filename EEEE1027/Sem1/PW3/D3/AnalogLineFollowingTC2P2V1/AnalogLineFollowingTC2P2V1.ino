...
void loop() {
  ...
  // Simulate reaching the top of the ramp, waiting for 4 seconds, and doing the 360
  reachedTop = true;
  topWaitEnd = true;
  hasSpun = true;
  ...
  if (reachedTop == true && topWaitEnd == true && hasSpun == true && rampDescent == false) {
    if (minPitch >= RAMP_ANGLE_NEG) {
      lineFollowingLogic(159, leftSensor, rightSensor);
    }
    ...
    if (minPitch < RAMP_ANGLE_NEG && accel_z > 10.4 && pitch > -1.0) {
      ...
      rampDescent = true;
      // Store the distance travelled up to this point to be used as a comparison point
      currDistTravelled = distance;
      // Set the startPhase flag to be true to indicate that the car should now
      // be in the "Follow the path for 80 cm" section of the second part of TC2
      startPhase = true;
    }
  }

  if (rampDescent == true && startPhase == true && stopPhase == false && (distance - currDistTravelled) <= 80) {
    lineFollowingLogic(107, leftSensor, rightSensor);
    // moveForward(180);
  }
  ...
  if (rampDescent == true && stopPhase == true && startPhase == false && (elapsedTime - currStopTime) <= STOP_PHASE_DUR) { moveStop(); }
  if (rampDescent == true && stopPhase == true && startPhase == false && (elapsedTime - currStopTime) > STOP_PHASE_DUR) {
    stopPhase = false;
    startPhase = true;
    currDistTravelled = distance;
  }
