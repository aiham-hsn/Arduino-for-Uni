...
const unsigned long TOP_WAIT_DURATION = 4000;
...
const int START_MARK_NUDGE = 2;   // distance in cm to travel forward by so that the robot is aligned with the 0cm start mark
...
void loop() {
  ...
  if (reachedTop == true && topWaitEnd == true && hasSpun == true && rampDescent == false) {
    ...
    if (minPitch < RAMP_ANGLE_NEG && accel_z > 10.4 && pitch > -1.0) {
      moveStop();
      // Set flag to indicate that the robot has descended the ramp
      rampDescent = true;
