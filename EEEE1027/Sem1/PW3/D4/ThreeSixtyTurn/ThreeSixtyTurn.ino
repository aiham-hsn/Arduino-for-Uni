...
//// Common constants
double rad2deg = (double)180 / (double)PI;  // Convert whatever it's being multiplied with from radians to degrees
...
void lineFollowingLogic(int normalSpeed, int leftSensor, int rightSensor) {
  ...
  if (leftSensor < IR_THRESHOLD && rightSensor > IR_THRESHOLD) { moveLeft(HI_SPD); }
...
void rotate360(int SPEED, unsigned long ROTATE_TIMEOUT) {
...
void loop() {
  ...
  if (rampAscent == false && reachedTop == false) {
    // Simulate having reached the top of the ramp
    rampAscent = true;
    reachedTop = true;

    // Record the time at which the car reaches the top as a reference
    currStopTime = elapsedTime;
  }

  // Wait for 4 seconds at the top of the ramp
  if (reachedTop == true && topWaitEnd == false && hasSpun == false && (elapsedTime - currStopTime) < TOP_WAIT_DURATION) { moveStop(); }
